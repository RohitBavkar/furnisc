...
export async function createCheckoutSession(
  ...
  try {
    ...
    // 3. Fetch current product data from Prisma to validate prices/stock
    ...
    const { stripeCustomerId, prismaCustomerId } =
    ...
    const metadata = {
      ...
      userEmail,
      prismaCustomerId,
      productIds: validatedItems.map((i) => i.product.id).join(","),
